  ...
  function save() {
    if (instance_of($this->new_revision, 'Revision')){
      ...
      if (($this->isColumnModified('project_index') && $this->getColumnValue('project_index') == 1)){
        $sql = 'UPDATE ' . $this->getTableName() . ' SET `project_index` = 0 WHERE `project_id` = ' . $this->getProjectId();
        DB::execute($sql);
      }
      ...
      if (($this->isColumnModified('project_sidebar') && $this->getColumnValue('project_sidebar') == 1)){
        $sql = 'UPDATE ' . $this->getTableName() . ' SET `project_sidebar` = 0 WHERE `project_id` = ' . $this->getProjectId();
        DB::execute($sql);
      }
